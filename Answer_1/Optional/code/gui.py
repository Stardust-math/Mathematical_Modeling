...
class MetroApp:
    ...
    def _build_sidebar(self, sidebar: ttk.Frame):
        ...
        self.transfer_options_frame = ttk.Frame(sec)
        self.transfer_options_frame.pack(fill=tk.X, pady=(0, 8))

        self.use_transfer_var = tk.BooleanVar(value=False)
        self.transfer_check = ttk.Checkbutton(
            self.transfer_options_frame,
            text="Consider transfer time",
            variable=self.use_transfer_var,
            command=self._update_transfer_controls_visibility,
        )
        self.transfer_check.pack(anchor=tk.W, pady=(0, 4))

        self.transfer_input_frame = ttk.Frame(self.transfer_options_frame)
        ttk.Label(self.transfer_input_frame, text="Transfer Cost:").pack(side=tk.LEFT)
        self.transfer_time_var = tk.StringVar(value="5.0")
        ttk.Entry(self.transfer_input_frame, textvariable=self.transfer_time_var, width=10).pack(
        ...
        self._update_transfer_controls_visibility()

    def _update_transfer_controls_visibility(self):
        city = self.city_var.get()
        show_transfer_checkbox = (city == "Beijing")

        if show_transfer_checkbox:
            if not self.transfer_options_frame.winfo_ismapped():
                self.transfer_options_frame.pack(fill=tk.X, pady=(0, 8), before=self.btn_frame)
        else:
            self.use_transfer_var.set(False)
            self.transfer_input_frame.pack_forget()
            self.transfer_options_frame.pack_forget()
            return

        if self.use_transfer_var.get():
            if not self.transfer_input_frame.winfo_ismapped():
                self.transfer_input_frame.pack(fill=tk.X)
        else:
            self.transfer_input_frame.pack_forget()
    ...
    def _on_solve(self):
        ...
        use_transfer = (self.metro.city == "Beijing") and bool(self.use_transfer_var.get())

        try:
            transfer_time = float(self.transfer_time_var.get())
        except ValueError:
            self._status_var.set("Transfer cost must be a number")
            self._log("Transfer cost must be a valid number.")
            return

        cost, path = self.metro.shortest_path(
            src_name,
            dst_name,
            use_transfer_time=use_transfer,
        ...
        cost_label = "Total Cost" if use_transfer else "Distance"
        unit = "" if use_transfer else " km"

        self._draw_path(path, cost, cost_label=cost_label, unit=unit)
        self.canvas.draw_idle()
        self._status_var.set(f"Route found: {len(path)} stations, {cost:.3f}{unit}")
        self._log(
            "{line}\n"
            "  {src}  ->  {dst}\n"
            "  {cost_label} : {cost:.3f}{unit}\n"
            "  Stops       : {stops}\n"
            "  Route       : {route}\n"
            "{extra}"
            "{line}".format(
                ...
                stops=len(path),
                route=" -> ".join(names),
                extra=(f"  Transfer Cost Setting : {transfer_time:.3f}\n" if use_transfer else ""),
            )
        )
    ...
    def _on_reset(self):
        ...
        self.dst_var.set("")
        self.use_transfer_var.set(False)
        self.transfer_time_var.set("5.0")
        self._update_transfer_controls_visibility()
    ...
    def _draw_path(
        ...
        self.ax.set_title(
            "{city} Metro Network\n"
            "Shortest Path: {src} -> {dst}\n({cost_label} = {cost:.3f}{unit})".format(
