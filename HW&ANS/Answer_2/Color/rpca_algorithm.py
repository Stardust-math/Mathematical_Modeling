...
def rpca_alm_color(
    ...
    """
    if A.ndim == 2:
        return rpca_alm(A, lam=lam, tol=tol, max_iter=max_iter)
    ...
    for c in range(3):
        Lc, Sc, info_c = rpca_alm(A[:, :, c], lam=lam, tol=tol, max_iter=max_iter)
...
class ImageRestorer:
    ...
    def solve(
        ...
        """执行 RPCA 分解。"""
        L, S, info = rpca_alm_color(self.original, lam=lam, tol=tol, max_iter=max_iter)
