...
from rpca_algorithm import ImageRestorer, detect_images, make_masked_input_display

matplotlib.use("TkAgg")


class RPCAApp:
    """基于 Tkinter 的掩膜 TV-RPCA 图像修复界面。"""
    ...
    def __init__(self, fig_root: str | Path):
        ...
        self.damage_mask: np.ndarray | None = None
        self.mask_boxes: list[tuple[int, int, int, int]] = []
        self._preview_rects: list[Rectangle] = []
        self._mask_selecting = False
        self._mask_select_start_data: tuple[float, float] | None = None
        self._mask_select_anchor_ax = None

        self.current_axes = []
        self.current_panel_keys: list[str] = []
        self._base_limits = []
    ...
    def _configure_styles(self):
        ...
        for key, (style_name, color_value) in self.CHANNEL_STYLE.items():
            style.configure(
                style_name,
                troughcolor="#f1f5f9",
                background=color_value,
                bordercolor="#f1f5f9",
                lightcolor=color_value,
                darkcolor=color_value,
            )

    def _build_ui(self):
        self.root = tk.Tk()
        self.root.title("RPCA Image Restoration")
        self.root.geometry("1580x950")
        ...
        self.mask_mode_var = tk.BooleanVar(master=self.root, value=False)
        ...
        self.observed_ratio_var = tk.StringVar(master=self.root, value="-")
        self.mask_box_var = tk.StringVar(master=self.root, value="0")
        ...
        ttk.Label(
            title_block,
            text="Masked TV-RPCA with manual damage selection and interactive visualization.",
    ...
    def _build_zoom_toolbar(self, parent: ttk.Frame):
        ...
        ttk.Button(bar, text="Zoom Out", command=lambda: self._set_zoom(self._zoom_value / 1.25)).grid(row=0, column=0, rowspan=2, padx=(0, 6), sticky="ns")
        ttk.Button(bar, text="Fit View", command=self._reset_zoom).grid(row=0, column=1, rowspan=2, padx=(0, 10), sticky="ns")
        ...
        scale = ttk.Scale(bar, orient=tk.HORIZONTAL, variable=self._zoom_var, from_=self.ZOOM_MIN, to=self.ZOOM_MAX, command=self._on_zoom_slider)
    ...
    def _build_sidebar_contents(self, sidebar: ttk.Frame):
        ...
        mask_sec = ttk.LabelFrame(sidebar, text="Mask Tools", style="Section.TLabelframe", padding=10)
        mask_sec.pack(fill=tk.X, padx=px, pady=py)

        self.mask_check = ttk.Checkbutton(mask_sec, text="Box Select Damage Region", variable=self.mask_mode_var, command=self._on_mask_mode_toggle)
        self.mask_check.pack(anchor="w", pady=(0, 8))

        ttk.Label(
            mask_sec,
            text="Drag a rectangle on the original or observed panel.",
            style="Panel.TLabel",
            wraplength=self.SIDEBAR_WIDTH - 90,
            justify=tk.LEFT,
        ).pack(anchor="w")

        mask_row = ttk.Frame(mask_sec, style="Panel.TFrame")
        mask_row.pack(fill=tk.X, pady=(8, 0))
        mask_row.columnconfigure(0, weight=1)
        mask_row.columnconfigure(1, weight=1)

        self.undo_mask_btn = ttk.Button(mask_row, text="Undo Box", command=self._undo_mask_box)
        self.undo_mask_btn.grid(row=0, column=0, sticky="ew", padx=(0, 4))

        self.clear_mask_btn = ttk.Button(mask_row, text="Clear Mask", command=self._clear_mask_boxes)
        self.clear_mask_btn.grid(row=0, column=1, sticky="ew", padx=(4, 0))
        ...
        self.run_btn = ttk.Button(run_row, text="Run Masked TV-RPCA", style="Accent.TButton", command=self._on_solve)
        ...
        ttk.Label(loading_box, textvariable=self._loading_status_var, style="Info.TLabel").pack(anchor="w", padx=8, pady=(2, 6))
        ...
        self.save_original_btn = ttk.Button(save_grid, text="Save Original", command=lambda: self._save_single_image("original", "original"), state=tk.DISABLED)
        self.save_original_btn.grid(row=0, column=0, sticky="ew", padx=(0, 4), pady=(0, 6))

        self.save_input_btn = ttk.Button(save_grid, text="Save Masked Input", command=lambda: self._save_single_image("masked_input", "masked_input"), state=tk.DISABLED)
        self.save_input_btn.grid(row=0, column=1, sticky="ew", padx=(4, 0), pady=(0, 6))

        self.save_lowrank_btn = ttk.Button(save_grid, text="Save L", command=lambda: self._save_single_image("low_rank", "low_rank"), state=tk.DISABLED)
        self.save_lowrank_btn.grid(row=1, column=0, sticky="ew", padx=(0, 4))

        self.save_sparse_btn = ttk.Button(save_grid, text="Save S", command=lambda: self._save_single_image("sparse", "sparse"), state=tk.DISABLED)
        ...
        self._build_info_block(info_sec, "Observed Ratio", self.observed_ratio_var)
        self._build_info_block(info_sec, "Mask Boxes", self.mask_box_var)
    ...
    def _build_channel_progress_row(self, parent: ttk.Frame, channel_name: str):
        ...
        title_label = ttk.Label(header, textvariable=label_var, style="Info.TLabel", font=("Segoe UI Semibold", 10))
        title_label.grid(row=0, column=0, sticky="w")

        state_label = ttk.Label(header, textvariable=state_var, style="Muted.TLabel")
        state_label.grid(row=0, column=1, sticky="e")

        style_name, _ = self.CHANNEL_STYLE[channel_name]
        ...
        ttk.Label(row_box, textvariable=detail_line1, style="Muted.TLabel", justify=tk.LEFT).pack(anchor="w", padx=8)
        ttk.Label(row_box, textvariable=detail_line2, style="Muted.TLabel", justify=tk.LEFT).pack(anchor="w", padx=8, pady=(0, 6))
    ...
    def _build_info_block(self, parent: ttk.Frame, label_text: str, value_var: tk.StringVar):
        ...
        ttk.Label(box, textvariable=value_var, style="Info.TLabel", justify=tk.LEFT, wraplength=self.SIDEBAR_WIDTH - 100).grid(row=1, column=0, sticky="w", padx=8, pady=(2, 6))
    ...
    def _on_sidebar_mousewheel(self, event):
        ...
        if widget is None or not self._is_descendant_of(widget, self.sidebar_frame):
            return
    ...
    def _refresh_images(self):
        ...
        if self.images:
            self.image_var.set(current_name if current_name in self.images else self.images[0])
        else:
            self.image_var.set("")
    ...
    def _browse_image(self):
        file_path = filedialog.askopenfilename(
            ...
            filetypes=[("Image Files", "*.png *.jpg *.jpeg *.bmp *.tif *.tiff"), ("PNG Files", "*.png"), ("JPEG Files", "*.jpg *.jpeg"), ("Bitmap Files", "*.bmp"), ("All Files", "*.*")],
        )
        if not file_path:
            return
        path = Path(file_path)
    ...
    def _save_figure(self):
        if not self.current_axes:
            ...
            return
        default_name = "masked_tv_rpca.png" if self.restorer is None else f"{self.restorer.image_path.stem}_masked_tv_rpca.png"
    ...
    def _save_single_image(self, result_key: str, suffix: str):
        if self.current_result is None or result_key not in self.current_result:
            return
        default_stem = "rpca" if self.restorer is None else self.restorer.image_path.stem
        file_path = filedialog.asksaveasfilename(
            title="Save image panel",
            defaultextension=".png",
            initialfile=f"{default_stem}_{suffix}.png",
            filetypes=[("PNG", "*.png"), ("JPEG", "*.jpg *.jpeg"), ("Bitmap", "*.bmp"), ("TIFF", "*.tif *.tiff"), ("All Files", "*.*")],
        )
        if not file_path:
            return
    ...
    def _load_image(self, image_path: Path):
        ...
        self.damage_mask = self.restorer.default_mask()
        self.mask_boxes = []
    ...
    def _on_mask_mode_toggle(self):
        self._status_var.set("Mask mode: drag rectangles on the left panels" if self.mask_mode_var.get() else "Mask mode off")

    def _clear_mask_boxes(self):
        if self.damage_mask is None:
            return
        self.mask_boxes = []
        self.damage_mask[:] = 1.0
        self.current_result = None
        self._set_result_save_buttons_state(False)
        self._remove_preview_rects()
        self._draw_original()
        self._update_image_info()
        self._log("Mask cleared.")

    def _undo_mask_box(self):
        if self.damage_mask is None or not self.mask_boxes:
            return
        self.mask_boxes.pop()
        self._rebuild_mask_from_boxes()
        self.current_result = None
        self._set_result_save_buttons_state(False)
        self._remove_preview_rects()
        self._draw_original()
        self._update_image_info()
        self._log("Last mask box removed.")

    def _rebuild_mask_from_boxes(self):
        if self.damage_mask is None:
            return
        self.damage_mask[:] = 1.0
        for r0, r1, c0, c1 in self.mask_boxes:
            self.damage_mask[r0:r1, c0:c1] = 0.0

    def _apply_mask_box_from_data(self, x0: float, y0: float, x1: float, y1: float):
        if self.damage_mask is None:
            return
        h, w = self.damage_mask.shape
        col0 = max(0, min(w, int(np.floor(min(x0, x1) + 0.5))))
        col1 = max(0, min(w, int(np.ceil(max(x0, x1) + 0.5))))
        row0 = max(0, min(h, int(np.floor(min(y0, y1) + 0.5))))
        row1 = max(0, min(h, int(np.ceil(max(y0, y1) + 0.5))))
        if row1 <= row0 or col1 <= col0:
            return
        self.mask_boxes.append((row0, row1, col0, col1))
        self.damage_mask[row0:row1, col0:col1] = 0.0
        self.current_result = None
        self._set_result_save_buttons_state(False)
        self._draw_original()
        self._update_image_info()
        self._log(f"Added mask box: rows [{row0}, {row1}), cols [{col0}, {col1})")
    ...
    def _on_solve(self):
        ...
        mask_copy = None if self.damage_mask is None else self.damage_mask.copy()
        self._set_busy(True)
        self._status_var.set("Running Masked TV-RPCA in background...")
        self._start_loading_animation("Computing")
        self._reset_channel_progress_display(max_iter=max_iter)
        self._log("Starting Masked TV-RPCA computation...")
                ...
                if kind == "progress":
                    ...
                    for item in info.get("channel_infos", []):
                        self._update_channel_progress(
                            item["channel_name"],
                            item["iterations"],
                            max(int(self.max_iter_var.get()), item["iterations"]),
                            item["error"],
                            item["rank"],
                            item["sparse_ratio"],
                            "Finished",
                    ...
                    self._log(f"Observed Ratio: {info['observed_ratio']:.4f}")
                    ...
                    self._status_var.set("Masked TV-RPCA failed")
                    self._stop_loading_animation("Failed")
                    self._log(f"Masked TV-RPCA failed: {payload}")
                    messagebox.showerror("Masked TV-RPCA", f"Masked TV-RPCA failed:\n{payload}")
                    self._set_busy(False)
        except queue.Empty:
            pass
        self.root.after(120, self._poll_worker)
    ...
    def _set_busy(self, busy: bool):
        ...
        for widget in [self.run_btn, self.load_btn, self.browse_btn, self.refresh_btn, self.save_btn, self.reset_btn, self.undo_mask_btn, self.clear_mask_btn, self.mask_check]:
            widget.configure(state=state)
        self.image_cb.configure(state="disabled" if busy else "readonly")
        self.run_btn.configure(text="Running..." if busy else "Run Masked TV-RPCA")
    ...
    def _reset_channel_progress_display(self, max_iter: int = 0):
        ...
        if channel_mode == "Gray":
            ...
            if not self._channel_rows["R"].winfo_manager():
                self._channel_rows["R"].pack(fill=tk.X, pady=4)
            ...
            self._channel_bars["R"].configure(style=self.CHANNEL_STYLE["Gray"][0], maximum=max(max_iter, 1), value=0)
            return

        for name in self._channel_order:
            if not self._channel_rows[name].winfo_manager():
                self._channel_rows[name].pack(fill=tk.X, pady=4)
            ...
            self._channel_bars[name].configure(style=self.CHANNEL_STYLE[name][0], maximum=max(max_iter, 1), value=0)

    def _update_channel_progress(self, channel_name: str, iteration: int, max_iter: int, error: float, rank, sparse_ratio: float, state_text: str):
        ...
        self._channel_detail_line1_vars[key].set(f"Iteration {iteration} / {max_iter}    Error {error:.3e}")
        self._channel_detail_line2_vars[key].set(f"Rank {rank}             Sparse {sparse_ratio:.4f}")
        self._channel_bars[key].configure(maximum=max(max_iter, 1), value=min(max(iteration, 0), max(max_iter, 1)))
    ...
    def _on_figure_press(self, event):
        if event.inaxes not in self.current_axes or event.button != 1:
            return
        if event.x is None or event.y is None:
            return
        if self.mask_mode_var.get() and event.inaxes in self.current_axes[:2]:
            self._mask_selecting = True
            self._mask_select_anchor_ax = event.inaxes
            self._mask_select_start_data = (event.xdata, event.ydata)
            self._remove_preview_rects()
            self._add_preview_rect(event.inaxes, event.xdata, event.ydata, event.xdata, event.ydata)
            return
        self._dragging = True
        self._drag_started = False
        self._drag_anchor_ax = event.inaxes
        self._drag_start_pixels = (event.x, event.y)
        self._drag_start_views = [(ax.get_xlim(), ax.get_ylim()) for ax in self.current_axes]

    def _on_figure_release(self, event):
        if self._mask_selecting:
            if self._mask_select_start_data is not None and event.xdata is not None and event.ydata is not None:
                x0, y0 = self._mask_select_start_data
                self._apply_mask_box_from_data(x0, y0, event.xdata, event.ydata)
            self._mask_selecting = False
            self._mask_select_start_data = None
            self._mask_select_anchor_ax = None
            self._remove_preview_rects()
            return
        self._dragging = False
    ...
    def _on_figure_motion(self, event):
        if self._mask_selecting:
            if self._mask_select_start_data is None or event.xdata is None or event.ydata is None or self._mask_select_anchor_ax is None:
                return
            x0, y0 = self._mask_select_start_data
            self._update_preview_rect(self._mask_select_anchor_ax, x0, y0, event.xdata, event.ydata)
            self.canvas.draw_idle()
            return
    ...
    def _add_preview_rect(self, ax, x0, y0, x1, y1):
        rect = Rectangle((min(x0, x1), min(y0, y1)), abs(x1 - x0), abs(y1 - y0), fill=False, lw=2.0, ec="#ef4444")
        ax.add_patch(rect)
        self._preview_rects = [rect]

    def _update_preview_rect(self, ax, x0, y0, x1, y1):
        if not self._preview_rects:
            self._add_preview_rect(ax, x0, y0, x1, y1)
            return
        rect = self._preview_rects[0]
        rect.set_x(min(x0, x1))
        rect.set_y(min(y0, y1))
        rect.set_width(abs(x1 - x0))
        rect.set_height(abs(y1 - y0))

    def _remove_preview_rects(self):
    ...
    def _to_rgb_display(self, image: np.ndarray) -> np.ndarray:
        if image.ndim == 2:
            img = np.clip(image, 0.0, 1.0)
            return np.stack([img, img, img], axis=2)
        return np.clip(image, 0.0, 1.0).copy()

    def _compose_display_image(self, image: np.ndarray, show_mask_overlay: bool = False, show_mask_boxes: bool = False) -> np.ndarray:
        display = self._to_rgb_display(image)

        if show_mask_overlay and self.damage_mask is not None:
            masked = self.damage_mask < 0.5
            if np.any(masked):
                overlay_color = np.array([0.93, 0.20, 0.20], dtype=np.float64)
                alpha = 0.22
                display[masked] = (1.0 - alpha) * display[masked] + alpha * overlay_color

        if show_mask_boxes and self.mask_boxes:
            border_color = np.array([0.86, 0.15, 0.15], dtype=np.float64)
            thickness = 2
            h, w = display.shape[:2]
            for r0, r1, c0, c1 in self.mask_boxes:
                rr0 = max(0, min(h, r0))
                ...
                top = slice(rr0, min(rr0 + thickness, rr1))
                bottom = slice(max(rr1 - thickness, rr0), rr1)
                left = slice(cc0, min(cc0 + thickness, cc1))
                right = slice(max(cc1 - thickness, cc0), cc1)

                display[top, cc0:cc1] = border_color
                display[bottom, cc0:cc1] = border_color
                display[rr0:rr1, left] = border_color
                display[rr0:rr1, right] = border_color

        return np.clip(display, 0.0, 1.0)

    def _show_image(self, ax, image, title: str, show_mask_overlay: bool = False, show_mask_boxes: bool = False):
        display = self._compose_display_image(image, show_mask_overlay=show_mask_overlay, show_mask_boxes=show_mask_boxes)
        ax.imshow(display)
    ...
    def _draw_original(self):
        ...
        masked_input = make_masked_input_display(original, self.damage_mask)

        self._show_image(ax1, original, "Original Image", show_mask_overlay=True, show_mask_boxes=True)
        self._show_image(ax2, masked_input, "Observed Region", show_mask_boxes=True)

        self.current_axes = [ax1, ax2]
        self.current_panel_keys = ["original", "masked_input"]
    ...
    def _draw_result(self, result: dict):
        ...
        self._show_image(ax1, result["original"], "Original Image", show_mask_overlay=True, show_mask_boxes=True)
        self._show_image(ax2, result["masked_input"], "Observed Region", show_mask_boxes=True)
        self._show_image(ax3, result["low_rank"], "Completed Low-rank L")
        self._show_image(ax4, result["sparse"], "Sparse Part S")

        self.current_axes = [ax1, ax2, ax3, ax4]
        self.current_panel_keys = ["original", "masked_input", "low_rank", "sparse"]
        self.fig.tight_layout(pad=2.1)
        self._capture_base_limits()
        self.canvas.draw()
        self._reset_zoom()
    ...
    def _update_image_info(self):
        if self.restorer is None:
            ...
            self.observed_ratio_var.set("-")
            self.mask_box_var.set("0")
            return
        ...
        self.mask_box_var.set(str(len(self.mask_boxes)))

        observed_ratio = 1.0 if self.damage_mask is None else float(np.mean(self.damage_mask))
        self.observed_ratio_var.set(f"{observed_ratio:.4f}")
        ...
        self.observed_ratio_var.set(f"{info.get('observed_ratio', observed_ratio):.4f}")
