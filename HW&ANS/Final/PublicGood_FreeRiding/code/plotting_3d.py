...
def _save_3d(fig, relpath_no_ext: str, dpi: int = 450) -> None:
    ...
    # 3D axes are easily cropped by tight_layout. Explicit margins and padding
    # keep axis labels and colorbars visible in the final PDF.
    fig.subplots_adjust(left=0.04, right=0.86, bottom=0.04, top=0.92)
    fig.savefig(str(out) + '.svg', bbox_inches='tight', pad_inches=0.24)
    fig.savefig(str(out) + '.png', dpi=dpi, bbox_inches='tight', pad_inches=0.24)
    plt.close(fig)
...
def plot_policy_response_surface_3d(df: pd.DataFrame, relpath: str = 'figs/synthetic/fig_policy_response_surface_3d') -> None:
    """Plot subsidy-penalty response surface with welfare as height and average maintenance pressure as color."""
    ...
    ax.set_zlabel('Average tail welfare', labelpad=12)
    ax.set_title('Subsidy–Penalty Response Surface', pad=14, fontsize=13, fontweight='bold')
...
def plot_pareto_front_3d(df: pd.DataFrame, relpath: str = 'figs/synthetic/fig_pareto_front_3d') -> None:
    ...
    ax.set_zlabel('Average maintenance pressure', labelpad=12)
    ax.set_title('3D Projection of the Pareto Policy Trade-off', pad=14, fontsize=13, fontweight='bold')
...
def plot_dynamic_phase_3d(df: pd.DataFrame, relpath: str = 'figs/synthetic/fig_dynamic_phase_3d') -> None:
    ...
    ax.set_zlabel('Social welfare W(t)', labelpad=12)
    ax.set_title('Dynamic Stock–Pressure–Welfare Trajectories', pad=14, fontsize=13, fontweight='bold')
