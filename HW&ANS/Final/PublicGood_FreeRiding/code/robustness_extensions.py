...
def plot_behavioral_robustness(one_step: pd.DataFrame, policy_summary: pd.DataFrame) -> None:
    ...
    if one_step is not None and not one_step.empty:
        ...
        fig.tight_layout()
        fig.savefig(out / 'synthetic_behavioral_weight_robustness.png', dpi=300)
        fig.savefig(out / 'synthetic_behavioral_weight_robustness.svg')
        plt.close(fig)
    ...
    if policy_summary is not None and not policy_summary.empty:
        ...
        fig.tight_layout()
        fig.savefig(out / 'synthetic_policy_ranking_under_behavioral_weights.png', dpi=300)
        fig.savefig(out / 'synthetic_policy_ranking_under_behavioral_weights.svg')
        plt.close(fig)
...
def plot_one_at_a_time_behavioral_weight_robustness(oat: pd.DataFrame) -> None:
    ...
    fig.tight_layout()
    fig.savefig(out / 'synthetic_behavioral_weight_oat_underprovision.png', dpi=300)
    fig.savefig(out / 'synthetic_behavioral_weight_oat_underprovision.svg')
    plt.close(fig)
    ...
    ax.legend(loc='best', frameon=True, fontsize=8)
    fig.tight_layout()
    fig.savefig(out / 'synthetic_behavioral_weight_oat_welfare_loss.png', dpi=300)
    fig.savefig(out / 'synthetic_behavioral_weight_oat_welfare_loss.svg')
    plt.close(fig)
...
def plot_damping_robustness(damping: pd.DataFrame) -> None:
    ...
    ax.legend(loc='best', frameon=True)
    fig.tight_layout()
    fig.savefig(out / 'synthetic_solver_damping_robustness.png', dpi=300)
    fig.savefig(out / 'synthetic_solver_damping_robustness.svg')
    plt.close(fig)
