...
class SocialPlanner:
    def __init__(self, agents: pd.DataFrame, params: Dict[str, float], contribution_max: float, free_rider_threshold: float = 0.08, high_benefit_quantile: float = 0.55):
        ...
        self.a = self.agents['efficiency'].to_numpy(float)
        self.b = self.agents['benefit'].to_numpy(float)
        self.c = np.maximum(self.agents['cost'].to_numpy(float), 1e-8)
        self.mu = self.agents['pressure_sensitivity'].to_numpy(float)
        self.high_benefit_cutoff = float(np.quantile(self.b, high_benefit_quantile))
        self.q_max = float(np.sum(self.a * self.contribution_max))
        self.last_result = None

    def _free_flags(self, effort: np.ndarray) -> np.ndarray:
        return (effort < self.free_rider_threshold) & (self.b >= self.high_benefit_cutoff)
