...
def simulate_policy_trajectory(agents: pd.DataFrame, scenario_params: Dict, policy: Dict, config: Dict, scenario: str, policy_name: str, mode: str = 'nash', rng=None) -> pd.DataFrame:
    ...
    eff_array = [float(v) for v in agents['efficiency'].to_list()]
    model = PublicGoodDynamicModel(scenario_params, q_max=float(sum(v * contribution_max for v in eff_array)))
    ...
    for t in range(T):
        ...
        effort_list = [float(v) for v in effort]
        Q = float(sum(ei * xi for ei, xi in zip(eff_array, effort_list)))
        ...
        rows.append({
            ...
            'avg_effort': float(sum(effort_list) / len(effort_list)), 'effort_gini': gini(effort_list),
...
def one_step_equilibrium_summary(agents: pd.DataFrame, scenario_params: Dict, policy: Dict, config: Dict, scenario: str) -> Tuple[dict, dict, dict]:
    ...
    for mode, effort in [('nash', e_nash), ('social', e_social)]:
        ...
        rows.append({
            ...
            'avg_effort': float(sum(effort_list) / len(effort_list)), 'effort_gini': gini(effort_list), 'free_riding_ratio': float(sum(bool(v) for v in free) / len(free)),
